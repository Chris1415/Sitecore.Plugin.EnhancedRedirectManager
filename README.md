...
- **Dashboard Widget** on a site dashboard. Eight at-a-glance real-data tiles (Maps / Mappings / 301 / 302 / Server Transfer / Avg per map / Largest map / Last updated), plus header badges (all-healthy placeholder + real source-URL collisions count), a top-destinations bar list, and a recently-shipped-maps panel. Site picker at the top right (the SDK does not surface "current site" today; pick once and the widget remembers it via `localStorage`). Wide-variant layout (≥960px) reflows to three or four columns; small-variant collapses to a single column.
